...
export const useSquadBuilder = () => {
    ...
    const updatePlayerPosition = (sourceIndex: number, destinationIndex: number, sourceDroppableId: PlayerListKey, destinationDroppableId: PlayerListKey) => {
    ...
    const setListById = (id: PlayerListKey, list: Player[]) => {
        const setters: { [key in PlayerListKey]: Dispatch<SetStateAction<Player[]>> } = { attack: setAttack, mid: setMid, def: setDef, gk: setGk, substitutes: setSubstitutes, reserves: setReserves };
        setters[id](list);
    };
